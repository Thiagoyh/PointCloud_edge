...
pcl::RangeImage& PointCloud::trans_cloud_to_image(const pcl::PointCloud<pcl::PointXYZ> point_cloud)
{
    ...
    range_image.createFromPointCloud(point_cloud, 0.00872665,
...
void PointCloud::BorderExtractor(const pcl::RangeImage range_image)
{
    ...
    res_veil.header.frame_id = "kinect_frame_optical";
    res_veil.header.stamp = ros::Time::now();
    res_shadow.header.frame_id = "kinect_frame_optical";
    res_shadow.header.stamp = ros::Time::now();
    res_pub.publish(res_border);
    veil_pub.publish(res_veil);
    shadow_pub.publish(res_shadow);
}
